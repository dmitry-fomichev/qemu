                                     ...
                                     uint32_t nlb, NvmeReadFillCtx *rfc)
{
    ...
    if (status != NVME_SUCCESS) {
         ...
         */
        next_zone = zone + 1;
        status = nvme_check_zone_state_for_read(next_zone);
        if (status != NVME_SUCCESS) {
            ;
...
static void nvme_auto_transition_zone(NvmeNamespace *ns, bool adding_active)
{
    NvmeZone *zone;
...
static uint16_t nvme_auto_open_zone(NvmeNamespace *ns, NvmeZone *zone)
{
    ...
    if (zs == NVME_ZONE_STATE_EMPTY) {
        nvme_auto_transition_zone(ns, true);
        status = nvme_aor_check(ns, 1, 1);
    } else if (zs == NVME_ZONE_STATE_CLOSED) {
        nvme_auto_transition_zone(ns, false);
