...
static uint16_t nvme_cmd_effects(NvmeCtrl *n, uint8_t csi, uint32_t len,
                                 uint64_t off, NvmeRequest *req)
{
    NvmeCmd *cmd = &req->cmd;
    uint64_t prp1 = le64_to_cpu(cmd->dptr.prp1);
    ...
    return nvme_dma_prp(n, (uint8_t *)&cmd_eff_log, len, prp1, prp2,
                        DMA_DIRECTION_FROM_DEVICE, req);
}

static uint16_t nvme_changed_zones(NvmeCtrl *n, bool rae, int8_t csi,
                                   uint32_t len, uint64_t off, NvmeRequest *req)
{
    NvmeCmd *cmd = &req->cmd;
    uint64_t prp1 = le64_to_cpu(cmd->dptr.prp1);
    uint64_t prp2 = le64_to_cpu(cmd->dptr.prp2);
    ...
    return nvme_dma_prp(n, (uint8_t *)&zc_log, len, prp1, prp2,
                        DMA_DIRECTION_FROM_DEVICE, req);
}
...
static uint16_t nvme_get_log(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    case NVME_LOG_ZONE_CHANGED_LIST:
        return nvme_changed_zones(n, rae, csi, len, off, req);
    default:
...
static Property nvme_props[] = {
    ...
    DEFINE_PROP_UINT64("zone_size", NvmeCtrl, params.zone_size_mb, 512),
    DEFINE_PROP_UINT64("zone_capacity", NvmeCtrl, params.zone_capacity_mb, 512),
